...
def knn_seeker(generated_data, enlarged_data):
    ...
    # Reshape to 2d array [n,100,71]
    # enlarged_data = np.reshape(enlarged_data, [enl_no, seq_len * dim])
    # generated_data = np.reshape(generated_data, [gen_no, seq_len * dim])
    enlarged_data = np.squeeze(enlarged_data[:,:,0])
    generated_data = np.squeeze(generated_data[:,:,0]) # Take out the time dimensions
    
    # Output initialization
    distance = np.zeros([enl_no,])

    # For each data entry in enlarge dataset
    for i in range(enl_no):
        temp_dist = list()
        # For each generated data entry
