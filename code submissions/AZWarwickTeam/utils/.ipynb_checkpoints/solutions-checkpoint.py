...
def validate_hider_output(
    ...
    try:
        ...
        if raise_exception:
            raise RuntimeError(
                "Problem loading hider generated data. "  # pylint: disable=raise-missing-from
...
def validate_seeker_output(seeker: str, seeker_output_path: str, labels: np.ndarray, raise_exception=True):
    ...
    if os.path.isfile(seeker_output_path):
        try:
            ...
            if raise_exception:
                raise RuntimeError(
                    "Problem loading hider-vs-seeker evaluation results. "  # pylint: disable=raise-missing-from
