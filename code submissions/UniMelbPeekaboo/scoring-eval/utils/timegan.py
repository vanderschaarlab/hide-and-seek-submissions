"""Time-series Generative Adversarial Networks (TimeGAN) Codebase with Differential Privacy.

UniMelb Peekaboo Team
Last updated: December 24 2020
Differentially private (DP) training of TimeGan
Code edits: training of disciminator D is done in a differentially private manner.
...
with a few changes from the original timegan provided
"""
...
def timegan(ori_data):
    ...
    # 20000 iterations will take way too long. After some experiments, we found the loss converged around 4000 iterations. Therefore, we finally choose
    # 4000 iterations for training.
    ...
    # Code for differentially private training of discriminator D.
    # =========================================================================
    optimizer = tf.train.AdamOptimizer() # choose the optimiser
    D_grad_vars = optimizer.compute_gradients(D_loss, var_list = d_vars) # take out the gradient
    C = 1 # hyperparameter control clipping norm
    sigma = 0.1 # hyperparameter control noise level
    D_grad_vars = [(tf.divide(grad, tf.maximum(tf.constant(1.), tf.divide(tf.norm(grad), tf.constant(C, TF_FLOAT)))) + tf.random_normal(grad.shape, 0, sigma * C), var) for grad, var in D_grad_vars] # Clipping, normalising and noising the gradient
    D_solver = optimizer.apply_gradients(D_grad_vars) # put the processed gradient back
    # =========================================================================
