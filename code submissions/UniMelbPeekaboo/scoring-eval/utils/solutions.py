...
def parse_hider_output(hider_output: Union[Tuple, np.ndarray]):
    ...
    exc_message = (
        ...
        "(b) (generated_data: np.ndarray, padding_mask: np.ndarray / None)."
    )
    if isinstance(hider_output, (tuple, np.ndarray)):
        if isinstance(hider_output, np.ndarray):
            return hider_output, None
        else:
            if (
                len(hider_output) == 2
                ...
                return hider_output[0], hider_output[1]
            else:
...
def benchmark_hider(
    ...
    rmse_threshold = (1 - threshold) + 1

    # Feature prediction.
    print("\nBenchmark feature prediction scores:")
    ...
    for idx, eval_score in enumerate(eval_feat_scores):
        ...
        if eval_type == "classification":
            if soln_score < auc_threshold * eval_score:
                print(
                    "Feature prediction: "
                    f"evaluation feature {idx + 1}. {soln_score} < {auc_threshold} * {eval_score}."
                )
                failed = True
        else:
            if soln_score > rmse_threshold * eval_score:
                print(
                    "Feature prediction: "
                    f"evaluation feature {idx + 1}. {soln_score} > {rmse_threshold} * {eval_score}."
                )
                failed = True

    # OSA.
    if osa_score > rmse_threshold * eval_osa_score:
        print(f"One-step-ahead prediction score: {osa_score} > {rmse_threshold} * {eval_osa_score}.")
        failed = True
