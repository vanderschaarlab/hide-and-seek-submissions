...
def batch_generator(batch_size, *data):
    ...
    Args:
        ...
        - data: data to take batches of. Each entry must have same length

    Returns:
        - *args_mb batch_size samples from each
    """
    no = len(data[0])
    ...
    D_mb = tuple(list(d[i] for i in train_idx) for d in data)

    return D_mb

def batch_generator_notime(data, batch_size):
    """Mini-batch generator.
...
def random_generator_notime(batch_size, z_dim):
    """Random vector generation.
    ...
    Z_mb = list()
    for i in range(batch_size):
        temp_Z = np.random.uniform(0.0, 1, [z_dim])
        Z_mb.append(temp_Z)
    return Z_mb