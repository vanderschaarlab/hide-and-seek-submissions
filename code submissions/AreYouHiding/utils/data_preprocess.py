...
#from .gain_pytorch import gain_main
from scipy import stats

PADDING_FILL = -1.0


def get_medians_and_scaler(data: np.ndarray, padding_mask: np.ndarray):
    ...
    return scaler, median_vals

def fill_quantile(col, percentage=0.25):
    val = stats.mstats.mquantiles(col, prob=percentage)[0]
    col = col.clip(lower=0)
    col = col.fillna(0)
    # print(val)
    return col.replace(to_replace=[0, np.nan], value=val)
...
def load_and_reshape(
    ...
    # print("Start imputing with GAIN in PyTorch")
    # ori_data = gain_main(ori_data)
    # print("Finish missing data imputation")
    ...
    for i in trange(1, len(ori_data.columns)):
        ori_data.iloc[:, i] = fill_quantile(ori_data.iloc[:, i])
    ...
    for key in keys:
        ...
        mode_vals.append(stats.mode(val).mode[0])

    scaler, _ = get_medians_and_scaler(loaded_data, padding_mask)
    _, data_imputed = process_and_impute(loaded_data, padding_mask, scaler, mode_vals)

    return data_imputed, padding_mask
...
def process_and_impute(
    data: np.ndarray, padding_mask: np.ndarray, scaler: MinMaxScaler, median_vals: np.ndarray
) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for i in range(data.shape[0]):
        ...
        cur_data_imputed = imputation(cur_data, median_vals)

        # Set time
        cur_data[:, 0] = preprocessed_time
        cur_data_imputed[:, 0] = preprocessed_time
...
def imputation(curr_data: np.ndarray, median_vals: np.ndarray, zero_fill: bool = True) -> np.ndarray:
    ...
    if most_frequent:
        ...
        imputed_data = imp_fre.fit_transform(curr_data)
...
def preprocess_data(data: np.ndarray, padding_mask: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    preproc_data, imputed_data = process_and_impute(data, padding_mask, scaler, median_vals)
    return preproc_data, imputed_data
