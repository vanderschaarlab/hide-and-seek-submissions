...
class TimeGAN(tf.keras.Model):
    def __init__(self, args):
        ...
        self.mse = tf2.keras.losses.MeanSquaredError(reduction=tf2.keras.losses.Reduction.NONE)
    ...
    def recovery_forward(self, X, optimizer):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            E_loss_T0 = (tf.math.reduce_mean(loss_fn())/10)**2
            var_list = self.embedder.trainable_weights + self.recovery.trainable_weights
            grads = optimizer.compute_gradients(loss_fn, var_list, gradient_tape=tape)
...
def train_timegan(ori_data, mode='train'):
    ...
    with open('examples/hider/timegan/params.json', 'r') as f:
    ...
    if mode == 'train':
        ...
        #for itt in range(1):
            ...
            step_e_loss = model.recovery_forward(X_mb, dp_optimizer)
            if itt % 100 == 0:
